...
[System.IO.File]::WriteAllText($linker,
    "@echo off`r`n`"$clang`" -target aarch64-linux-ohos --sysroot=`"$sysroot`" -Wl,-soname,libgpui_mobile.so %*`r`n")
...
$library = Join-Path $env:CARGO_TARGET_DIR 'aarch64-unknown-linux-ohos\release\libgpui_mobile.so'
$destination = Join-Path $PSScriptRoot 'entry\libs\arm64-v8a\libgpui_mobile.so'
