...
impl WgpuContext {
    ...
    fn select_color_texture_format(adapter: &wgpu::Adapter) -> anyhow::Result<wgpu::TextureFormat> {
        ...
        // The OHOS Vulkan driver reports BGRA support, but samples from color
        // glyphs uploaded to a BGRA atlas as transparent on tested hardware.
        #[cfg(target_env = "ohos")]
    ...
    pub fn uses_webgl_instance_data(&self) -> bool {
        (matches!(self.backend, WgpuBackend::Gl) && cfg!(target_family = "wasm"))
            || (cfg!(target_env = "ohos")
                && matches!(self.backend, WgpuBackend::Native(wgpu::Backend::Gl))
                && !self
                    .adapter
